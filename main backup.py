...
# #CHANGE MADE
# from transcriber_to_gesture import transcribe_continuously
...
# def video_player():
#     sigml_file = 'SignFiles/B.sigml'
#     return render_template('play.html', sigml_file=sigml_file)




# @app.route('/transcribe-and-translate')
# def transcribe_and_translate():
#     clear_all()
#     output = transcribe_continuously()

#     # Filter out previously processed sentences
#     new_sentences = [s for s in output if s not in processed_sentences]
#     processed_sentences.update(new_sentences)
...
#         result = process_pipeline(input_text)
...
import time
import ssl
from temp import transcribe_video_to_chunks
...
def get_word_sigml_mapping(transcript_text):
    ...
    for word in words:
        ...
        if os.path.exists(full_path):
            ...
            current_time += 3  # or any custom delay
        else:
...
def video_player():
    ...
    try:
        ...
        # Step 2: Process the transcript input
        take_input(transcript_text)  # This will internally call convert(), pre_process(), convert_to_final()
